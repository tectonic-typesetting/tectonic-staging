...
void parsearguments(void)
{
    ...
    do {
#ifdef TECTONIC_STAGE4_UNDERSCORES
        getoptreturnval = getopt_long_only(argc, argv, "", longoptions, addressof(optionindex));
#else
        getoptreturnval = getoptlongonly(argc, argv, "", longoptions, addressof(optionindex));
#endif
...
#else
            printversionandexit("This is OTANGLE, Version 4.4", nil, "J. Plaice, Y. Haralambous, D.E. Knuth", nil);
#endif
        }
    }
...
void send_out(eightbits t, sixteenbits v)
{
 ...
 restart:
    ...
    case SIGN_VAL_VAL:
        if (t == FRAC ||
            ((t == IDENT && v == 3 && ((outcontrib[1] == ASCII_D
                                       ...
                                       (outcontrib[1] == ASCII_m
                                        && outcontrib[2] == ASCII_o && outcontrib[3] == ASCII_d)
        ...
        if (c != IGNORE) {
            ...
            do {
                ...
                d = buffer[loc];
                        ...
                        ((d > ASCII_9) && (d < ASCII_A)) ||
                        ((d > ASCII_Z) && (d < ASCII_a)) || (d > ASCII_z)) && (d != UNDERSCORE)));
...
void mainbody(void)
{
    ...
    if (stringptr > NUMBER_CHARS) {
        putc('\n', stdout);
        fprintf(stdout, "%ld%s", (long)stringptr - NUMBER_CHARS, " strings written to string pool file.");
...
{
    if (n >= argc) {            /* This error message should never happen, because the callers
