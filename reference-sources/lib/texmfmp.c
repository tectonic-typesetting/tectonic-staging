...
/* The C version of the job_name, if given. */
...
/* maininit, called from main() - this is most of the main routine,
   ...
   we set up for that in `t_open_in' (which is called from the .web).  */
...
/* main: Set up for reading the command line, which will happen in
   `maininit' and `t_open_in', then call the main body, plus
...
struct msg
{
  int   name_length; /* length of auxiliary data */
...
#ifdef IPC_DEBUG
  fprintf(stderr, "%d\t%d\n", ourmsg.msg.name_length, ourmsg.msg.eof);
  fputs ("tex: Sending message to socket ...\n", stderr);
#endif
  ourmsg.msg.name_length = n;
...
ipc_page (int is_eof)
{
  ...
  if (!begun) {
    ...
    ipc_open_out ();
#if !defined(Aleph)
    len = str_start[output_file_name + 1] - str_start[output_file_name];
#else
    len = strstartar[output_file_name + 1 - 65536L] -
            strstartar[output_file_name - 65536L];
#endif
    name = xmalloc (len + 1);
#if !defined(Aleph)
    strncpy (name, (string)&str_pool[str_start[output_file_name]], len);
#else
    {
    unsigned i;
    for (i=0; i<len; i++)
#if IS_pTeX
      name[i] =  0xFF&str_pool[i+str_start[output_file_name]];
#else
      name[i] =  str_pool[i+strstartar[output_file_name - 65536L]];
#endif
    }
#endif
    name[len] = 0;
...
parse_options (int argc, string *argv)
{
  ...
  for (;;) {
    ...
    } else if (ARGUMENT_IS ("job_name")) {
#ifdef XeTeX
      c_job_name = optarg;
#else
      c_job_name = normalize_quotes (optarg, "job_name");
#endif
    ...
    /* For backward compatibility only. */
      ...
      if (!set_enc_string (NULL, optarg)) {
        ...
        versions = NULL;
#endif
        print_version_and_exit (BANNER, COPYRIGHT_HOLDER, AUTHOR, versions);
...
open_in_or_pipe (FILE **f_ptr, int filefmt, const_string fopen_mode)
{
    ...
    return open_input_impl(f_ptr,filefmt,fopen_mode) ;
}

#ifdef XeTeX
boolean
u_open_in_or_pipe(unicode_file* f, integer filefmt, const_string fopen_mode, integer mode, integer encodingData)
{
    string fname = NULL;
    int i; /* iterator */
    ...
    return u_open_in_impl(f, filefmt, fopen_mode, mode, encodingData);
}
#endif
...
void
u_close_file_or_pipe (unicode_file* f)
{
  int i; /* iterator */
...
#if IS_pTeX
  for (i = last+1; (i < last + 5 && i < buf_size) ; i++)
    buffer[i] = '\0';
#endif
...
#ifdef TeX
    if (input_stack[is_ptr].state_field == 0 /* token list */
        || input_stack[is_ptr].name_field <= 255) { /* can't be filename */
#elif defined(MF)
    if (input_stack[is_ptr].name_field <= 255) {
#else
#error "Unable to identify program" /* MetaPost doesn't use this file */
#endif
        ; /* fprintf (stderr, "call_edit: skipped input_stack[%d], ", is_ptr);
             fprintf (stderr, "name_field=%d <= 255 or state_field=%d == 0\n",
                      input_stack[is_ptr].name_field,
                      input_stack[is_ptr].state_field); */
    } else {
      FILE *f;
...
#if !defined(pdfTeX)
static void
checkpoolpointer (pool_pointer pool_ptr, size_t len)
{
...
#endif
  len = strlen(s);
  checkpoolpointer (pool_ptr, len); /* in the XeTeX case, this may be more than enough */
#ifdef XeTeX
  while ((rval = *(cp++)) != 0) {
    UInt16 extraBytes = bytesFromUTF8[rval];
    switch (extraBytes) { /* note: code falls through cases! */
...
{
    str_number ret = name; int i, l, p;
    if (c_job_name != NULL)
      ret = maketexstring(c_job_name);
#if IS_pTeX
    i = str_start[ret]; l = str_start[ret+1];
    while (i<l)
     {
        p = multistrlenshort(str_pool, l, i);
        if (p>1) {
             int j;
             for (j=i+p; i<j; i++) str_pool[i] = (0xFF&str_pool[i])+0x100;
        } else i++;
     }
#endif /* IS_pTeX */
    return ret;
...
#if IS_pTeX
  for (i=0; i<len; i++) name[i] =  0xFF&str_pool[i+str_start[s]];
#else
  for (i=0; i<len; i++) name[i] =  str_pool[i+strstartar[s - 65536L]];
#endif
  }
#endif
  name[len] = 0;
...
{
  pool_pointer oldpoolptr = pool_ptr;
...
void
getmd5sum(str_number s, boolean file)
{
    md5_state_t state;
...
void
blank_rectangle (screencol left, screencol right,
...
void
paint_row (screenrow row, pixelcolor init_color,
